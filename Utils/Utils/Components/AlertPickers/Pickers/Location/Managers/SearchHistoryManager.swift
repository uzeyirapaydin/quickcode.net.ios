...
struct SearchHistoryManager {
	...
	func history() -> [LocationAP] {
		let history = defaults.object(forKey: HistoryKey) as? [NSDictionary] ?? []
		return history.compactMap(LocationAP.fromDefaultsDic)
	}
	
	func addToHistory(_ location: LocationAP) {
...
extension LocationAP {
	...
	class func fromDefaultsDic(_ dic: NSDictionary) -> LocationAP? {
		...
		return LocationAP(name: dic[LocationDicKeys.name] as? String,
