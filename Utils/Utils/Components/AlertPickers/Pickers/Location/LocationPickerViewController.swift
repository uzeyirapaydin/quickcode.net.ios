...
extension UIAlertController {
    ...
    public func addLocationPicker(location: LocationAP? = nil, completion: @escaping LocationPickerViewController.CompletionHandler) {
...
final public class LocationPickerViewController: UIViewController {
    ...
    public typealias CompletionHandler = (LocationAP?) -> ()
	...
	public var location: LocationAP? {
		didSet {
			if isViewLoaded {
	...
	func updateAnnotation() {
		...
		if let location = location {
            ...
            mapView.selectAnnotation(location as! MKAnnotation, animated: true)
		}
	}
    ...
    func selectLocation(location: CLLocation) {
        ...
        geocoder.reverseGeocodeLocation(location) { response, error in
            if let error = error as NSError?, error.code != 10 { // ignore cancelGeocode errors
                ...
                self.location = LocationAP(name: name, location: location, placemark: placemark)
...
extension LocationPickerViewController: UISearchResultsUpdating {
	...
	func showItemsForSearchResult(_ searchResult: MKLocalSearch.Response?) {
		results.locations = searchResult?.mapItems.map { LocationAP(name: $0.name, placemark: $0.placemark) } ?? []
		results.isShowingHistory = false
		results.tableView.reloadData()
	}
	
	func selectedLocation(_ location: LocationAP) {
