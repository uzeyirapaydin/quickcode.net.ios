...
class KMPopUpViewController: UIViewController {
    ...
    @available(iOS 9.0, *)
    func setupLayoute() {
        let imageWidth  = UIScreen.main.bounds.width * 0.88
        
        if witImage {
            MessageImage.widthAnchor.constraint(equalToConstant: imageWidth).isActive = true
            MessageImage.heightAnchor.constraint(equalToConstant: imageWidth).isActive = true
        } else {
        ...
        messageLable.widthAnchor.constraint(equalToConstant: imageWidth).isActive = true
